...
contains
    subroutine partial_rdf(A, B, precision, output_rdf)     ! calculates g_AB(r) using {precision} bins for the histogram
        ...
        !
        ! Count the atoms for normalisation
        total_number_B = 0
        total_number_A = 0
        do i = 1, N_part
        ...
        do i = 2, precision
            ...
            !
            Verif(i) = output_rdf(i,2)*total_number_B*volume_bin(i)/(Box_dimension(1)*Box_dimension(2)*Box_dimension(3)) ! check the normalization
    ...
    subroutine write_g_of_r(A,B,g_of_r,len_A,len_B)
        ...
        if (file_exists) then
            ...
            if ( Name_A /= Name_B .and. file_exists2 ) then
                ! Uncomment if you don't want to save RDFs for non-identical pairs
